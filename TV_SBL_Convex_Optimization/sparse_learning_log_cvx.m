function [x_sbl,gamma] = sparse_learning_log_cvx(A,Y,gamma,ite_sbl_max,beta_sbl,SigmaN,Diff_tol_sbl)
    [M N] = size(A);
    [M L] = size(Y);
    ...
    for k=1:ite_sbl_max
        ...
        for n = 2:N
            z_tv_cvx = z_tv_cvx + ( beta_sbl * abs( gamma_cvx(n) - gamma_cvx(n-1) ) / ( abs( gamma_prev(n) - gamma_prev(n-1) ) + eps1 ) );
        end
