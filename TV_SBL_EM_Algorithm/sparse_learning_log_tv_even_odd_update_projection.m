function [mu,dmu,k,gamma] = sparse_learning_log_tv_even_odd_update_projection(Phi,T,lambda,iters,flag1,flag2,flag3,beta_sbl,SNR,SigmaN)
...
while (1)
    ...
    elseif (flag1(1) == 4)        
        ...
        if (beta_sbl==0)
            ...
            c_i = q_i;
            c_i(2:end) = abs(gamma(2:end) - gamma(1:end-1)) + eps1;
            c_i(1) = gamma(1) + eps1;
            
            d_i = q_i;
            d_i(1:end-1) = abs(gamma(1:end-1) - gamma(2:end)) + eps1;
            d_i(end) = gamma(end) + eps1;
            ...
            a_i = 1./(abs(gamma - gamma_max) + eps1);
            %a_i(1) = a_i(1)/2;
            %a_i(M) = a_i(M)/2;
            b_i = 1./(abs(gamma - gamma_min) + eps1);
            %b_i(1) = b_i(1)/2;
            %b_i(M) = b_i(M)/2;

            
            % Unconstrained optimization update: With Majorization
            alpha_hat = real( max(0,sqrt(E_i./(q_i + beta_sbl.*(a_i + b_i) ))) );
            alpha_tilde = real( max(0,sqrt(E_i./max(eps2,q_i + beta_sbl.*(a_i - b_i) ) )) );
            alpha_bar = real( max(0,sqrt(E_i./max(eps2,q_i - beta_sbl.*(a_i + b_i) ) ) ) );
            ...
            for even_odd_index = 1:2
                ...
                %F1(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*log(abs(gamma_hat(2:end-1) - gamma(3:end) ) + eps1) + beta_sbl(2:end-1).*log(abs(gamma_hat(2:end-1) - gamma(1:end-2) ) + eps1);
                %F1(1) = f_i(1) + beta_sbl(1).*log(abs(gamma_hat(1) - gamma(2) ) + eps1);
                %F1(M) = f_i(M) + beta_sbl(M).*log(abs(gamma_hat(M) - gamma(M-1) ) + eps1);
                F1(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*(abs(gamma_hat(2:end-1) - gamma(3:end) )+eps1)./d_i(2:end-1) + beta_sbl(2:end-1).*(abs(gamma_hat(2:end-1) - gamma(1:end-2) )+eps1)./c_i(2:end-1);
                F1(1) = f_i(1) + beta_sbl(1).*(abs(gamma_hat(1) - gamma(2) )+eps1)/d_i(1);
                F1(M) = f_i(M) + beta_sbl(M).*(abs(gamma_hat(M) - gamma(M-1) )+eps1)/c_i(1);
                
                f_i = E_i./gamma_bar + q_i.*gamma_bar;
                %f_i = E_i./gamma_bar + log(gamma_bar + eps0);
                %F2(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*log(abs(gamma_bar(2:end-1) - gamma(3:end) ) + eps1) + beta_sbl(2:end-1).*log(abs(gamma_bar(2:end-1) - gamma(1:end-2) ) + eps1);
                %F2(1) = f_i(1) + beta_sbl(1).*log(abs(gamma_bar(1) - gamma(2) ) + eps1);
                %F2(M) = f_i(M) + beta_sbl(M).*log(abs(gamma_bar(M) - gamma(M-1) ) + eps1);
                F2(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*(abs(gamma_bar(2:end-1) - gamma(3:end) )+eps1)./d_i(2:end-1) + beta_sbl(2:end-1).*(abs(gamma_bar(2:end-1) - gamma(1:end-2) )+eps1)./c_i(2:end-1);
                F2(1) = f_i(1) + beta_sbl(1).*(abs(gamma_bar(1) - gamma(2) )+eps1)/d_i(1);
                F2(M) = f_i(M) + beta_sbl(M).*(abs(gamma_bar(M) - gamma(M-1) )+eps1)/c_i(1);
                
                f_i = E_i./gamma_tilde + q_i.*gamma_tilde;
                %f_i = E_i./gamma_tilde + log(gamma_tilde + eps0);
                %F0(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*log(abs(gamma_tilde(2:end-1) - gamma(3:end) ) + eps1) + beta_sbl(2:end-1).*log(abs(gamma_tilde(2:end-1) - gamma(1:end-2) ) + eps1);
                %F0(1) = f_i(1) + beta_sbl(1).*log(abs(gamma_tilde(1) - gamma(2) ) + eps1);
                %F0(M) = f_i(M) + beta_sbl(M).*log(abs(gamma_tilde(M) - gamma(M-1) ) + eps1);
                F0(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*(abs(gamma_tilde(2:end-1) - gamma(3:end) )+eps1)./d_i(2:end-1) + beta_sbl(2:end-1).*(abs(gamma_tilde(2:end-1) - gamma(1:end-2) )+eps1)./c_i(2:end-1);
                F0(1) = f_i(1) + beta_sbl(1).*(abs(gamma_tilde(1) - gamma(2) )+eps1)/d_i(1);
                F0(M) = f_i(M) + beta_sbl(M).*(abs(gamma_tilde(M) - gamma(M-1) )+eps1)/c_i(1);
...
%plot(q2/norm(q2),'r')
%plot(tv/norm(tv),'b')
%plot(gamma/norm(gamma),'k')
%legend('q','q2','tv','gamma')
