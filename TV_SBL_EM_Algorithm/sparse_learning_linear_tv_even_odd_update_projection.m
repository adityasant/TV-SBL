function [mu,dmu,k,gamma] = sparse_learning_linear_tv_even_odd_update_projection(Phi,T,lambda,iters,flag1,flag2,flag3,beta_sbl,SNR,SigmaN)
...
while (1)
    ...
    elseif (flag1(1) == 4)        
        ...
        if (beta_sbl==0)
            ...
            alpha_hat = real( max(0,sqrt(E_i./(q_i + 2*beta_sbl))) );
            alpha_tilde = real( max(0,sqrt(E_i./(q_i))) );
            alpha_bar = real( max(0,sqrt(E_i./max(eps1,q_i - 2*beta_sbl) ) ) );
           ...
           for even_odd_index = 1:2
                ...
                F1(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*abs(gamma_hat(2:end-1) - gamma(3:end) ) + beta_sbl(2:end-1).*abs(gamma_hat(2:end-1) - gamma(1:end-2) );
                F1(1) = f_i(1) + beta_sbl(1).*abs(gamma_hat(1) - gamma(2) );
                F1(M) = f_i(M) + beta_sbl(M).*abs(gamma_hat(M) - gamma(M-1) );

                f_i = E_i./gamma_bar + q_i.*gamma_bar;
                %f_i = E_i./gamma_bar + log(gamma_bar + eps0);
                F2(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*abs(gamma_bar(2:end-1) - gamma(3:end) ) + beta_sbl(2:end-1).*abs(gamma_bar(2:end-1) - gamma(1:end-2) );
                F2(1) = f_i(1) + beta_sbl(1).*abs(gamma_bar(1) - gamma(2) );
                F2(M) = f_i(M) + beta_sbl(M).*abs(gamma_bar(M) - gamma(M-1) );

                f_i = E_i./gamma_tilde + q_i.*gamma_tilde;
                %f_i = E_i./gamma_tilde + log(gamma_tilde + eps0);
                F0(2:end-1) = f_i(2:end-1) + beta_sbl(2:end-1).*abs(gamma_tilde(2:end-1) - gamma(3:end) ) + beta_sbl(2:end-1).*abs(gamma_tilde(2:end-1) - gamma(1:end-2) );
                F0(1) = f_i(1) + beta_sbl(1).*abs(gamma_tilde(1) - gamma(2) );
                F0(M) = f_i(M) + beta_sbl(M).*abs(gamma_tilde(M) - gamma(M-1) );
